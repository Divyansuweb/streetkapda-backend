...
// ========== FALLBACK LOGGER - FIXES DEPLOYMENT ISSUE ==========
// This works even if logger.js is missing
let logger = console;
let morganStream = { write: (message) => console.log(message.trim()) };

try {
  const loggerModule = require('./utils/logger');
  if (loggerModule && loggerModule.logger) {
    logger = loggerModule.logger;
    morganStream = loggerModule.morganStream || morganStream;
    console.log('✅ Logger loaded successfully');
  } else {
    console.log('⚠️ Logger module loaded but no logger object, using console fallback');
  }
} catch (err) {
  console.log('⚠️ Logger module not found, using console fallback');
